...
for i in range(datalen):
    ...
    leftpatch =cv2.resize(leftpatch,(400,400))
    rightpatchwarped =cv2.resize(rightpatchwarped,(400,400))

    kpl,kpr,descl,descr,kpImageLeft,kpImageRight= descrfunc(leftpatch,rightpatchwarped)

    if len(kpl)<= 4 or len(kpr)<=4:
        i=i-1
        continue
    ...
    if(datavis == 1):
        ...
        plt.subplot(1,2,1)
        plt.imshow(kpImageLeft)
        plt.subplot(1,2,2)
        plt.imshow(kpImageRight)
        plt.show()


    descriptor_left = np.zeros((128,128))
    descriptor_right = np.zeros((128,128))
    if descl.shape[0] >= 128:
        descriptor_left = descl[:128]
    else:
        descriptor_left[:descl.shape[0]] = descl

    if descr.shape[0] >= 128:
        descriptor_right = descr[:128]
    else:
        descriptor_right[:descr.shape[0]] = descr

    if(datavis == 1):
        plt.subplot(1,2,1)
        plt.imshow(descriptor_left)
        plt.subplot(1,2,2)
        plt.imshow(descriptor_right)
        plt.show()
    ...
    trainingimage = np.dstack((descriptor_left,descriptor_right))
...
pickle.dump(train, open("./train_des.p", "wb"))
pickle.dump(valid, open("./valid_des.p", "wb"))
print("Done.")
